...
impl Attribute {
    ...
    fn common_attributes(key: &Key) -> anyhow::Result<HashMap<u64, Attribute>> {
        ...
        if let Some(cert) = key.openpgp_certificates().first() {
            ...
            // and post-quantum pieces with the t or pq suffix respectively.
            //
            // Finally, v4 ECDSA keys are documented as needing the hash algorithm and symmetric encryption
        ...
        match key.key_algorithm {
            ...
            KeyAlgorithm::Mldsa65 | KeyAlgorithm::Mldsa87 => {
                ...
                let parameter_set = match key.key_algorithm {
                    ...
                    algorithm => {
                        return Err(anyhow::anyhow!(
                            "Unsupported ML-DSA key algorithm {algorithm}"
                        ));
                    }
                };
                attrs.insert(
