...
impl Client {
    ...
    pub async fn new_user(&mut self, username: String, admin: bool) -> Result<User, ClientError> {
        let request = protocol::json::Request::NewUser { username, admin };
        let request = Request {
            message: request,
            binary: None,
        };
        let response = self.reconnecting_send(request).await?;
        match response {
            Response::NewUser { username, admin } => Ok(User {
                name: username,
                admin,
            }),
            Response::Error { reason } => Err(reason.into()),
