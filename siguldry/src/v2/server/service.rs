...
impl Server {
    ...
    pub async fn migrate(&self) -> anyhow::Result<()> {
        db::migrate(&self.db_pool).await?;

        Ok(())
    }

    // TODO replace with a CLI command
    pub async fn create_user(&self, name: &str, admin: bool) -> anyhow::Result<()> {
        let mut conn = self.db_pool.begin().await?;
        _ = db::User::create(&mut conn, name, admin).await?;
        conn.commit().await?;
        Ok(())
    }
