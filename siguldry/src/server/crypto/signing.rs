    ...
    for (digest_algorithm, hex_hash) in digests {
        ...
        let signature = match key.key_algorithm {
            KeyAlgorithm::Mldsa65 | KeyAlgorithm::Mldsa87 => {
                if !(pkey.is_a(openssl::pkey::KeyType::ML_DSA_65)
                    || pkey.is_a(openssl::pkey::KeyType::ML_DSA_87))
                {
                    return Err(anyhow::anyhow!(
                        "Key type claims to be ML-DSA-65 or ML-DSA-87, but isn't according to OpenSSL!"
                    ));
                }

                match digest_algorithm {
                    DigestAlgorithm::MldsaMu => {
                        // All this gross unsafe stuff can go away once the openssl crate stabilizes a high-level
                        // interface for signing with ML-DSA keys.

                        let mut context = openssl::md_ctx::MdCtx::new()?;
                        let pkey_context = context.digest_sign_init(None, pkey)?;
                        let mut mu_flag: std::ffi::c_int = 1;

                        // Safety:
                        // From OpenSSL 3.5's documentation for OSSL_PARAM:
                        //
                        // 1. OSSL_PARAM_construct_int creates a descriptor referring to the key
                        //    and C int storage, without taking ownership. `c"mu"` is a static,
                        //    NUL-terminated CStr. `&raw mut mu_flag` points to a live, aligned
                        //    c_int initialized to 1. `mu_flag` is neither moved nor modified
                        //    before the call to set_params below.
                        // 2. OSSL_PARAM_construct_end produces the null-key sentinel required to
                        //    terminate an OSSL_PARAM array.
                        //
                        // These calls form a properly terminated array of parameters and all
                        // data lives through its use by set_params.
                        let parameters = unsafe {
                            [
                                openssl_sys::OSSL_PARAM_construct_int(
                                    c"mu".as_ptr(),
                                    &raw mut mu_flag,
                                ),
                        ...
                        // Safety:
                        //
                        // EVP_PKEY_CTX_settable_params() requires an initialized context.
                        // `pkey_context` has been initialized via the call to digest_sign_init(),
                        // has not been used for a signing operation, and is exclusive so
                        // the pointer provided is safe for the function to read.
                        //
                        // OpenSSL returns null or a borrowed constant array of OSSL_PARAM
                        // structures that has been propery terminated. We not mutate or
                        // free the returned value.
                        let settable_parameters = unsafe {
                            openssl_sys::EVP_PKEY_CTX_settable_params(pkey_context.as_ptr())
                        };
                        if settable_parameters.is_null() {
                            return Err(anyhow::anyhow!(
                                "OpenSSL doesn't support any parameters when signing with ML-DSA \
                        ...
                        // Safety:
                        //
                        // OSSL_PARAM_locate_const() accepts a properly-terminated array of
                        // OSSL_PARAM structures and a valid C string key to search for. The
                        // array provided is from the array OpenSSL returned to us in the call
                        // to EVP_PKEY_CTX_settable_params(), which we have checked to be non-
                        // NULL. The key provided is a CStr with a static lifetime.
                        //
                        // The value returned is not mutated or freed.
                        let mu_parameter_unsupported = unsafe {
                            openssl_sys::OSSL_PARAM_locate_const(
                                settable_parameters,
                                c"mu".as_ptr(),
                            )
                            .is_null()
                        };
                        if mu_parameter_unsupported {
                            return Err(anyhow::anyhow!(
                                "OpenSSL doesn't support the Mu parameter when signing with ML-DSA"
                            ));
                        }

                        // Safety:
                        //
                        // EVP_PKEY_CTX_set_params() requires an initialized context and a properly
                        // terminated array of OSSL_PARAM structures. `pkey_context` has been
                        // initialized, has not been used for a signing operation, and is exclusive
                        // so the pointer provided is safe for the function to mutate.
                        //
                        // `parameters` is an array of OSSL_PARAM structures with the required
                        // terminating element, and all the data referenced by those structures
                        // has not moved, changed, or shared outside this scope. The data remains
                        // in that state as long as the `pkey_context`, so even if it stores a
                        // pointer to `mu_flag` rather than copying it, it remains valid for the
        ...
        let signature = match key.key_algorithm {
            ...
            _ => return Err(anyhow::anyhow!("Unknown key type requested!")),
        };
