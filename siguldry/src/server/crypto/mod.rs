...
use sequoia_openpgp::{
    Profile,
    crypto::{Password, mem::Protected, mpi},
    packet,
...
fn generate_password() -> anyhow::Result<Password> {
    let mut buf = Protected::new(128);
    openssl::rand::rand_priv_bytes(&mut buf)?;
...
pub struct EncryptedKey {
    ...
    pub openpgp_certificate: Option<String>,
}

#[derive(Default)]
pub struct KeyCertificateParameters {
    pub x509: Option<X509CertificateParameters>,
    pub openpgp: Option<OpenPgpCertificateParameters>,
}

pub struct X509CertificateParameters {
    pub usage: KeyUsage,
    pub common_name: String,
    pub validity_days: NonZeroU32,
}

pub struct OpenPgpCertificateParameters {
    pub user_id: packet::UserID,
    pub profile: Profile,
    pub hash_algorithm: sequoia_openpgp::types::HashAlgorithm,
...
fn create_key(algorithm: KeyAlgorithm) -> anyhow::Result<PKey<Private>> {
    ...
    // There's a fairly horrible gotcha with ML-DSA keys and PKCS#8, where they can be
    // encoded several ways. We use APIs that require the seed to be available, which
    // is not guaranteed to be true depending on OpenSSL configurations[0]. Round-trip
    // those keys to PKCS#8 here to ensure they include the seed.
    //
    // [0]
    // See https://docs.openssl.org/3.5/man7/EVP_PKEY-ML-DSA/#provider-configuration-parameters
    match algorithm {
        KeyAlgorithm::Mldsa65 | KeyAlgorithm::Mldsa87 => {
            let password = generate_password()?;
            let pkcs8 = encrypt_key(password.clone(), &key)?;
            let key =
                password.map(|p| PKey::private_key_from_pem_passphrase(pkcs8.as_bytes(), p))?;
            mldsa_seed(&key)?;
        }
        _ => {}
    }
...
// There's a bunch of options for serializing an ML-DSA key to PKCS#8, but we expect
// seed to be available, not just the secret key *sk*. Extract the seed into a type
// that will be scrubbed on Drop and provides error context.
fn mldsa_seed(key: &PKey<Private>) -> anyhow::Result<Protected> {
    let mut seed = Protected::new(32);
    let seed_length = match key.seed_into(&mut seed) {
        Ok(seed_length) => seed_length,
        Err(error) => {
            tracing::error!(
                ?error,
                "Failed to extract the ML-DSA seed from the PKCS#8-serialized key. \
                Set `ml-dsa.output_formats = seed-priv` in the OpenSSL configuration used \
                by Siguldry, then recreate the key."
            );
            return Err(anyhow::anyhow!(
                "Set 'ml-dsa.output_formats = seed-priv': {:?}",
                error
            ));
        }
    };
...
pub fn create_encrypted_key(
    ...
    let x509_certificate = x509_options
        ...
        .transpose()?;
    let openpgp_certificate = openpgp_options
        .map(|parameters| {
...
fn openssl_keys_to_openpgp(
    ...
    if let Ok(rsa) = traditional_openssl_key.rsa() {
        ...
        if let Some(hybrid_key) = pqc_openssl_key {
            ...
            if !hybrid_key.is_a(openssl::pkey::KeyType::ML_DSA_65) {
                return Err(anyhow::anyhow!("Only ML-DSA-65 is supported with Ed25519"));
            }

            let mldsa_private_key_seed = mldsa_seed(hybrid_key)?;
        ...
        if let Some(hybrid_key) = pqc_openssl_key {
            if !hybrid_key.is_a(openssl::pkey::KeyType::ML_DSA_87) {
                return Err(anyhow::anyhow!("Only ML-DSA-87 is supported with Ed448"));
            }
...
fn x509_certificate_for_key_private<P: HasPublic, S: HasPrivate>(
    pubkey: &PKey<P>,
    signing_key: &PKey<S>,
...
mod tests {
    ...
    fn mldsa_missing_seed_complains() -> anyhow::Result<()> {
        let temp_dir = tempfile::TempDir::new()?;
        let key_path = temp_dir.path().join("mldsa65.pem");
        let output = Command::new("openssl")
            .args([
                "genpkey",
                "-provparam",
                "ml-dsa.output_formats=priv-only",
                "-algorithm",
                "ML-DSA-65",
                "-out",
            ])
        ...
        assert!(
            ...
            "Failed to generate a seedless ML-DSA key: {}",
            String::from_utf8_lossy(&output.stderr)
        );

        let traditional_key = PKey::generate_ed25519()?;
        let mldsa_key = PKey::private_key_from_pem(&std::fs::read(key_path)?)?;
        openssl_keys_to_openpgp(&traditional_key, Some(&mldsa_key), Profile::RFC9580).unwrap_err();
        assert!(logs_contain("ml-dsa.output_formats = seed-priv"));
