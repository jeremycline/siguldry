...
use anyhow::Context;
use clap::Parser;
use siguldry::v2::server::{config::Config, service::Server};
...
/// These include applying database migrations, creating new administrators, and so on.
///
/// To begin, you'll need to provide a configuration file. For an example of the current
/// format, consult the `config` subcommand.
///
/// Once you have a valid configuration, create a new database using the `migrate` subcommand.
///
/// Finally, create an admin user with the `add-admin` subcommand.
#[derive(Debug, Parser)]
#[command(version)]
struct Cli {
    ...
    #[arg(long, short, env = "SIGULDRY_SERVER_CONFIG", value_parser = load_config)]
...
enum Command {
    ...
    ///
    /// Admins have write privileges and are required to add or remove other users, create keys,
    /// etc.
    AddAdmin {
...
async fn main() -> anyhow::Result<()> {
    ...
    match opts.command {
        ...
        Command::AddAdmin { name } => {
            let config = opts.config.unwrap_or_default();
            let server = Server::new(config).await?;
            server.create_user(&name, true).await?;
        }
    };
