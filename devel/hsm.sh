...
#
# The openssl stuff would need changes to work with the old engine interface, probably.
...
certutil -d nss -A -f ./nss_pwfile -n secure-boot-code-signer -t ,,u -i secure-boot-code-signing.crt
pesign -n nss -c "secure-boot-code-signer" \
	-R sample-uefi.sattrs.sig -I sample-uefi.sattrs.bin \
	--pinfile=./nss_pwfile \
	-i "$UNSIGNED_EFI" \
	-o signed.efi
sbverify --cert secure-boot-code-signing.crt signed.efi
...
pesign -n nss \
	...
	-i "$UNSIGNED_EFI" \
	-o signed2.efi
sbverify --cert secure-boot-code-signing.crt signed2.efi
